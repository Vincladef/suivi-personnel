...
async function main() {
  ...
  const uid = args.uid || process.env.REMINDER_UID
  const today = args.today || new Date().toISOString().slice(0, 10)
  const dryRun = args['dry-run'] === 'true'

  if (!uid) {
    throw new Error('Missing --uid <firebase uid>')
  }
  ...
  if (!document) {
    console.log(JSON.stringify({ ok: true, uid, sent: 0, reason: 'no-app-state' }))
    return
  }
  ...
  if (dueGoals.length === 0) {
    console.log(JSON.stringify({ ok: true, uid, sent: 0, reason: 'no-due-goals', today }))
    return
  }
  ...
  for (const goal of dueGoals) {
    ...
    if (!dryRun) {
      ...
      sent.push({ goalId: goal.id, title: goal.title, gmailId: result.id })
    } else {
      sent.push({ goalId: goal.id, title: goal.title, gmailId: 'dry-run' })
    }
  }

  console.log(JSON.stringify({ ok: true, uid, sent: sent.length, items: sent, today }))
}
