...
function sortTrackerItems(items: TrackerItem[]) {
...
function resequenceTrackerItems(items: TrackerItem[]) {
...
function App() {
  ...
  const habitItems = sortTrackerItems(state.trackerItems.filter((item) => item.module === 'habits'))
  const performanceItems = sortTrackerItems(state.trackerItems.filter((item) => item.module === 'performances'))
  ...
  function reorderTrackersWithinCategory(module: ModuleKey, category: string, draggedId: string, targetId: string) {
    ...
    const reorderedById = Object.fromEntries(reorderedBucket.map((item) => [item.id, item.order ?? 0]))
    patchState({ trackerItems: state.trackerItems.map((item) => item.module === module && reorderedById[item.id] != null ? { ...item, order: reorderedById[item.id] } : item) })
  }
                  ...
                  >
                    ...
                    <div className="tracker-category-list">
                      {grouped[category].map((item) => {
                        ...
                        return (
                          <article key={item.id} draggable onDragStart={() => setDraggedTrackerId(item.id)} onDragEnd={() => { setDraggedTrackerId(null); setDragOverTrackerId(null); dragHoverLockRef.current = '' }} onDragEnter={() => setDragOverTrackerId(item.id)} onDragLeave={() => { if (dragOverTrackerId === item.id) setDragOverTrackerId(null) }} onDragOver={(event) => { event.preventDefault() }} onDrop={() => { if (!draggedTrackerId || draggedTrackerId === item.id) return; const lockKey = `tracker-habits-${draggedTrackerId}-${item.id}`; if (dragHoverLockRef.current === lockKey) return; dragHoverLockRef.current = lockKey; reorderTrackersWithinCategory('habits', category, draggedTrackerId, item.id); setDraggedTrackerId(item.id); setDragOverTrackerId(null) }} className={`tracker-card is-draggable ${draggedTrackerId === item.id ? 'is-dragging' : ''} ${dragOverTrackerId === item.id ? 'is-drop-target' : ''} ${isCelebrating ? `is-celebrating celebration-level-${celebration.level}` : ''}`}>
                            {isCelebrating && (
          ...
          <section className="panel surface-panel">
            ...
            <div className="tracker-list">
              ...
              {visiblePerformanceItems.map((item) => {
                ...
                return (
                <article key={item.id} draggable onDragStart={() => setDraggedTrackerId(item.id)} onDragEnd={() => { setDraggedTrackerId(null); setDragOverTrackerId(null); dragHoverLockRef.current = '' }} onDragEnter={() => setDragOverTrackerId(item.id)} onDragLeave={() => { if (dragOverTrackerId === item.id) setDragOverTrackerId(null) }} onDragOver={(event) => { event.preventDefault() }} onDrop={() => { if (!draggedTrackerId || draggedTrackerId === item.id) return; const lockKey = `tracker-perf-${draggedTrackerId}-${item.id}`; if (dragHoverLockRef.current === lockKey) return; dragHoverLockRef.current = lockKey; reorderTrackersWithinCategory('performances', item.category || '', draggedTrackerId, item.id); setDraggedTrackerId(item.id); setDragOverTrackerId(null) }} className={`tracker-card is-draggable ${draggedTrackerId === item.id ? 'is-dragging' : ''} ${dragOverTrackerId === item.id ? 'is-drop-target' : ''} ${isCelebrating ? `is-celebrating celebration-level-${celebration.level}` : ''}`}>
                  {isCelebrating && (
                    <div key={celebration.token} className="dopamine-burst" aria-hidden="true">
                      {celebrationGlyphsForLevel(celebration.level).map((glyph, index) => (
                        <span key={`${glyph}-${index}`}>{glyph}</span>
                ...
                {monthLevelGoals.length > 0 && (
                  <div className="goal-list">
                    {monthLevelGoals.map((goal) => (
                      <article key={goal.id} draggable onDragStart={() => setDraggedGoalId(goal.id)} onDragEnd={() => { setDraggedGoalId(null); setDragOverGoalId(null); dragHoverLockRef.current = '' }} onDragEnter={() => setDragOverGoalId(goal.id)} onDragLeave={() => { if (dragOverGoalId === goal.id) setDragOverGoalId(null) }} onDragOver={(event) => { event.preventDefault() }} onDrop={() => { if (!draggedGoalId || draggedGoalId === goal.id) return; const lockKey = `goal-month-${draggedGoalId}-${goal.id}`; if (dragHoverLockRef.current === lockKey) return; dragHoverLockRef.current = lockKey; reorderGoalsWithinList(monthLevelGoals.map((entry) => entry.id), draggedGoalId, goal.id); setDraggedGoalId(goal.id); setDragOverGoalId(null) }} className={`goal-card is-draggable ${draggedGoalId === goal.id ? 'is-dragging' : ''} ${dragOverGoalId === goal.id ? 'is-drop-target' : ''} horizon-${goal.horizon} tone-${goalStatusTone(goal)} ${goalToneClass(goal)}`}>
                        <div className="goal-head">
                      ...
                      <section key={week.start} className="goal-week-block minimal-week-block" onClick={() => weekGoals.length === 0 ? openGoalModal(week.start) : undefined}>
                        ...
                        {weekGoals.length > 0 && (
                          <div className="goal-list compact-goal-list">
                            {weekGoals.map((goal) => (
                              <article key={goal.id} draggable onDragStart={() => setDraggedGoalId(goal.id)} onDragEnd={() => { setDraggedGoalId(null); setDragOverGoalId(null); dragHoverLockRef.current = '' }} onDragEnter={() => setDragOverGoalId(goal.id)} onDragLeave={() => { if (dragOverGoalId === goal.id) setDragOverGoalId(null) }} onDragOver={(event) => { event.preventDefault() }} onDrop={() => { if (!draggedGoalId || draggedGoalId === goal.id) return; const lockKey = `goal-week-${draggedGoalId}-${goal.id}`; if (dragHoverLockRef.current === lockKey) return; dragHoverLockRef.current = lockKey; reorderGoalsWithinList(weekGoals.map((entry) => entry.id), draggedGoalId, goal.id); setDraggedGoalId(goal.id); setDragOverGoalId(null) }} className={`goal-card is-draggable ${draggedGoalId === goal.id ? 'is-dragging' : ''} ${dragOverGoalId === goal.id ? 'is-drop-target' : ''} horizon-${goal.horizon} compact-goal-card tone-${goalStatusTone(goal)} ${goalToneClass(goal)}`}>
